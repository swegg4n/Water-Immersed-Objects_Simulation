...
static public class MeshColliderTools
{
    ...
    */
    ...
    public static void Simplify(this Mesh mesh)
    {
        ...
        var workingSet = new List<Vertex>(origNumVerts);
        ...
        HashSet<Vertex> nextCandidates = new HashSet<Vertex>();

        foreach (Vertex v in workingSet) if (!v.IsStatic)
                nextCandidates.Add(v);

        while (nextCandidates.Count != 0)
        {
            candidates = nextCandidates;
            nextCandidates = new HashSet<Vertex>();
            foreach (Vertex a in candidates)
            {
        ...
        foreach (Vertex v in workingSet) if (v.edges != null)
        ...
        foreach (Vertex v in simplifiedVerts)
        {
    ...
    private class Vertex
    {
        ...
        public bool IsStatic { get { return linearPosition == Vector3.zero; } }

        public Edge GetExistingConnectingEdge(Vertex v)
        {
        ...
        public bool CanFollow(Edge transportEdge, out Vertex opposite)
        {
            if (IsStatic) { opposite = default(Vertex); return false; }
            if (linearPosition != null && !transportEdge.IsParallel(linearPosition)) { opposite = default(Vertex); return false; }
    ...
    private class Edge
    {
        ...
        public void Reconnect(Vertex oldVertice, Vertex newVertice)
        {
    ...
    private class Face
    {
        ...
        public static void AddFace(List<Vertex> allVertices, int[] tris, int triIndex)
        {
        ...
        public Vertex GetOpposite(Edge o)
        {
            var o0 = o.vertices[0];
            var o1 = o.vertices[1];
            for (int i = 0; i < 3; i++)
            {
                Edge e = edges[i];
                Vertex e0 = e.vertices[0];
                if (e0 != o0 && e0 != o1) return e0;
                Vertex e1 = e.vertices[1];
        ...
        public bool MoveWouldFlip(Vertex v, Vector3 p)
        {
